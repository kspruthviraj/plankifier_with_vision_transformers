...
from vit_keras import vit # VIT KERAS
...
#			assert(len(trainY[0]) == self.numclasses)
	...
	def SetModelImage(self):
		...
		if self.modelname == 'mlp':
			...
			self.model = ResNet50Model.Build(input_shape=self.trainX[0].shape, classes=self.numclasses)   
		elif self.modelname == 'incepv3':
			self.model = InceptionV3Model.Build(input_shape=self.trainX[0].shape, classes=self.numclasses) 
		elif self.modelname == 'dense121':
			self.model = DenseNet121Model.Build(input_shape=self.trainX[0].shape, classes=self.numclasses) 
		elif self.modelname == 'smallvgg':
			self.model = SmallVGGNet.Build(input_shape=self.trainX[0].shape, classes=self.numclasses)
            
		elif self.modelname == 'b16':
			self.model = VitB16Model.Build(input_shape=self.trainX[0].shape, classes=self.numclasses)        
		elif self.modelname == 'b32':
			self.model = VitB32Model.Build(input_shape=self.trainX[0].shape, classes=self.numclasses)   
		elif self.modelname == 'l32':
			self.model = VitL32Model.Build(input_shape=self.trainX[0].shape, classes=self.numclasses)              
...
class MixedModel:
	...
	def Build(input_shape, classes, modelnames=['conv2','mlp'], nout=[32,32], ncombine=32, layers=[[128,64],[128,64]]):
		...
		elif modelnames[0] == 'b16':
			model_image = VitB16Model.Build(
				input_shape=input_shape[0], classes=nout[0])                   
		elif modelnames[0] == 'b32':
			model_image = VitB16Model.Build(
				input_shape=input_shape[0], classes=nout[0])             
		elif modelnames[0] == 'l32':
			model_image = VitB16Model.Build(
				input_shape=input_shape[0], classes=nout[0])
...
class VitB16Model:
	...
	def Build(input_shape, classes):
		...
		# initialize the model        
		base_model = vit.vit_b16(image_size = 128, activation = 'softmax', pretrained = True, include_top = False, pretrained_top = False, classes = classes)
        
		model = tf.keras.Sequential([base_model,
            tf.keras.layers.Flatten(),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.Dense(105, activation = tfa.activations.gelu),
            tf.keras.layers.Dropout(0.5),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.Dense(classes, 'softmax')],name = 'vit_b16')
		return model     
...
class VitB32Model:
	...
	def Build(input_shape, classes):
		...
		# initialize the model        
		base_model = vit.vit_b32(image_size = 128, activation = 'softmax', pretrained = True, include_top = False, pretrained_top = False, classes = classes)
		...
		model = tf.keras.Sequential([base_model,
            ...
            tf.keras.layers.Dropout(0.5),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.Dense(classes, 'softmax')],name = 'vit_b32')
		return model    
...
class VitL32Model:
	...
	def Build(input_shape, classes):
		...
		# initialize the model        
		base_model = vit.vit_l32(image_size = 128, activation = 'softmax', pretrained = True, include_top = False, pretrained_top = False, classes = classes)
		...
		model = tf.keras.Sequential([base_model,
            ...
            tf.keras.layers.Dropout(0.5),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.Dense(classes, 'softmax')],name = 'vit_l32')
		return model 

    
### Models for Bayesian search exploration ####    
...
class Explore_Mobile(HyperModel):
    ...
    def build(self, hp):
        ...
        model = tf.keras.Sequential([
            ...
            tf.keras.layers.Dropout(hp.Float('dropout1', 0.0, 0.7, step=0.1, default=0.5)),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.Dense(hp.Int('hidden_size', 50, 1000, step=100, default=50),
                                  activation='relu', bias_initializer='zeros'),
            tf.keras.layers.Dropout(hp.Float('dropout2', 0.0, 0.7, step=0.1, default=0.2)),
            tf.keras.layers.BatchNormalization(),
...
class Explore_VitB16(HyperModel):
    def __init__(self, input_shape, num_classes,loss):
        self.input_shape = input_shape
        self.num_classes = num_classes
        self.loss = loss
    ...
    def build(self, hp):
        base_model = vit.vit_b16(image_size = 128,activation = 'softmax',pretrained = True,
                        include_top = False,pretrained_top = False,classes = 35)
        ...
        model = tf.keras.Sequential([
            ...
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.Dense(hp.Int('hidden_size', 50, 1000, step=100, default=50),
                                  activation = tfa.activations.gelu, bias_initializer='zeros'),
            tf.keras.layers.Dropout(hp.Float('dropout2', 0.0, 0.7, step=0.1, default=0.2)),
            tf.keras.layers.BatchNormalization(),
            tf.keras.layers.Dense(self.num_classes, activation='softmax', 
                                  kernel_initializer='random_uniform', bias_initializer='zeros')
        ...
        model.compile(optimizer = tfa.optimizers.RectifiedAdam(learning_rate=1e-4),loss = self.loss, metrics = ['accuracy'])
        
        return model
    
    
    
class Explore_VitB32(HyperModel):
    def __init__(self, input_shape, num_classes,loss):
        self.input_shape = input_shape
        self.num_classes = num_classes
        self.loss = loss
    ...
    def build(self, hp):
        base_model = vit.vit_b32(image_size = 128,activation = 'softmax',pretrained = True,
                        include_top = False,pretrained_top = False,classes = 35)
...
class Explore_VitL32(HyperModel):
    def __init__(self, input_shape, num_classes,loss):
        self.input_shape = input_shape
        self.num_classes = num_classes
        self.loss = loss
    ...
    def build(self, hp):
        base_model = vit.vit_l32(image_size = 128,activation = 'softmax',pretrained = True,
                        include_top = False,pretrained_top = False,classes = 35)
...
def get_structure_of_best_models(X_train,classes,loss,outpath,model_to_train,max_trials,executions_per_trial,finetune):
    ...
    elif model_to_train=='b16':
        model = Explore_VitB16(input_shape=X_train[0].shape, num_classes=len(classes),loss=loss)
    elif model_to_train=='b32':
        model = Explore_VitB32(input_shape=X_train[0].shape, num_classes=len(classes),loss=loss)
    elif model_to_train=='l32':
        model = Explore_VitL32(input_shape=X_train[0].shape, num_classes=len(classes),loss=loss)         
    ...
    return tuner_Bayesian

            
            
def load_best_model(X_train,classes,loss,outpath,model_to_train,finetune, Mixed,init_name):
    
    tuner_Bayesian = get_structure_of_best_models(X_train,classes,loss,outpath,model_to_train,1,1,finetune)
    selected_hps=tuner_Bayesian.get_best_hyperparameters()[0]
    model = tuner_Bayesian.hypermodel.build(selected_hps)
...
def compile_and_train(model,outpath,model_to_train,learning_rate,
                      X_train,y_train,X_test,y_test,X_val,y_val,
...
#     print('Shape of X_train{}'.format(X_train.shape))
    ...
    if finetune ==1 and ttkind!='mixed': # Because for mixed we should not change the model parameters
        ...
        model=load_best_model(X_train,classes,loss,outpath,model_to_train,finetune=0,Mixed=Mixed,init_name=init_name)

    elif finetune ==2 and ttkind!='mixed': # Because for mixed we should not change the model parameters
        
        # load the tuned model
        model=load_best_model(X_train,classes,loss,outpath,model_to_train,finetune=1,Mixed=Mixed,init_name=init_name)# For second time finetuning               
...
#         layer.trainable = True
    ...
    model.compile(loss=loss,optimizer=tfa.optimizers.RectifiedAdam(learning_rate = learning_rate),metrics=["accuracy"])
...
def get_callbacks(bestmodelpath,patience,finetune):
    ...
    if finetune==0:
        reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(monitor = 'val_accuracy',factor = 0.2,patience = 2,verbose = 1,
    ...
    callbacks=[reduce_lr,checkpointer, logger,earlyStopping]
...
def combine_models(X_train,classes,loss,models_image,outpath,finetune,Mixed,init_name):
    members = list()
    
    for i in range(len(models_image)):
        members.append(load_best_model(X_train,classes,loss,outpath,foldername=models_image[i],
...
def avg_ensemble(X_test,y_test,X_val,y_val,
                 ...
                 finetune,Mixed,valid_set,for_mixed,init_name,loss):
    
    members=combine_models(X_test,classes,loss,models_image,outpath,finetune=finetune, Mixed=Mixed,init_name=init_name)
...
def combine_models_selected(X_train,classes,loss,models_image,outpath,finetune,Mixed,init_name):
    members = list()
...
def combine_models_across_iteration(X_train,classes,loss,model_image,outpath,finetune,Mixed,init_name):
    members = list()
    
    for i in range(len(init_name)):
        members.append(load_best_model(X_train,classes,loss,outpath,foldername=model_image,
                                       finetune=finetune,Mixed=Mixed,init_name=init_name[i]))
    return members


def avg_ensemble_selected_on_unlabelled(im_names,X_test,classes,models_image,outpath,
                          finetune,Mixed,for_mixed,init_name,path_to_save,loss):
    
    members=combine_models_selected(X_test,classes,loss,models_image,outpath,finetune,Mixed,init_name)
    Models_for_avg_ensemble = '_'.join(map(str, for_mixed))
    foldername='Ens_of_'+Models_for_avg_ensemble
...
def stacking_ensemble_selected_on_unlabelled(im_names,X_test,classes,models_image,outpath,
                      finetune,Mixed,init_name,stack_path,for_mixed,path_to_save,loss):
...
def stacking_ensemble_selected_on_labelled_test(X_test,y_test,classes,models_image,outpath,
                      finetune,Mixed,init_name,stack_path,for_mixed,ReportName,loss):
...
def avg_ensemble_iter_or_selected_models(X_test,y_test,X_val,y_val,classes,models_image,
                                      outpath,finetune,Mixed,valid_set,for_mixed,init_name,sel,ReportName,loss):
...
def stacking_ensemble_iter_or_selected_models(X_train,y_train,X_test,y_test,X_val,y_val,classes,models_image,
                                              outpath,finetune,Mixed,valid_set,for_mixed,init_name,sel,loss):
...
def Select_Mixed_Model(X_train,classes,loss,outpath,foldername,FeaturePath,finetune,Mixed,init_name):
    members = list()
    members.append(load_best_model(X_train,classes,loss,outpath,foldername,finetune,0,init_name)) # load from non-mixed model, i.e. Mixed=0
    members.append(load_best_model(X_train,classes,loss,outpath,FeaturePath,finetune,0,init_name)) # load from non-mixed model, i.e. Mixed=0
    return members
...
def Select_Mixed_Model_from_scratch(X_train,classes,ImageName):
    ...
    if ImageName == 'conv2' or ImageName == 'conv':
        ...
        members.append(SmallVGGNet.Build(input_shape=X_train[0][0].shape, 
                                              classes=len(classes), last_activation = 'softmax'))          
    elif ImageName == 'b16':
        members.append(VitB16Model.Build(input_shape=X_train[0][0].shape, classes=len(classes)))         
    elif ImageName == 'b32':
        members.append(VitB32Model.Build(input_shape=X_train[0][0].shape, classes=len(classes)))   
    elif ImageName == 'l32':
        members.append(VitL32Model.Build(input_shape=X_train[0][0].shape, classes=len(classes)))  
