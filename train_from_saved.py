...
class Ctrain:
	...
	def ReadArgs(self, string=None):
		...
		parser.add_argument('-model_image', choices=['conv2','smallvgg','mobile','eff0','eff1','eff2','eff3','eff4','eff5','eff6','eff7','incepv3','res50','dense121','b16','b32','l32'], default=None, help='For mixed data models, tells what model to use for the image branch. For image models, it is the whole model')
...
## For HyperParameter Tuning       
		...
		# Model related
		parser.add_argument('-models_image', nargs='*', default='mobile', help='select models to train from: conv,mobile,eff0,eff1,eff2,eff3,eff4,eff5,eff6,eff7,incepv3,res50,dense121,b16,b32,l32')
		# Ensemble related
				...
				if self.params.only_ensemble == 'no' or self.params.only_ensemble is None:
					...
					if self.params.avg_ensemble=='yes':
						hm.avg_ensemble(X_test=testX, y_test=testY,
                                        ...
                                        init_name=self.params.init_name,loss=self.loss)  
					...
					if self.params.stacking_ensemble=='yes':
						hm.stacking_ensemble(X_train=trainX, y_train=trainY,
                                             ...
                                             valid_set=self.params.valid_set,
                                             for_mixed=self.params.models_image,
                                             init_name=self.params.init_name,Mixed=0,loss=self.loss)
						...
						if self.params.stacking_ensemble=='yes':
							if len(Mixed_models_name) > 1: 
								hm.stacking_ensemble(X_train=[trainXimage,trainXfeat], 
                                                     ...
                                                     for_mixed=self.params.models_image,
                                                     init_name=self.params.init_name,Mixed=1,loss=self.loss)  
							else :
								print('For Ensemble, models selected should be greater than 1')
